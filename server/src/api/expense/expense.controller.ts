...
expenseRoute.post(
  ...
  async (c: Context) => {
    ...
    const data = createBulkExpenseSchema.parse(body);
    const result = await expenseService.insertBulk(data, payload.id);

    return c.json(
      Response.success(
        "Success create expense(s)",
        HTTP_STATUS.CREATED,
        result,
      ),
    );
...
expenseRoute.delete(
  ...
  async (c: Context) => {
    const payload = c.get("payload") as JWTPayload;
    const result = await expenseService.delete(c.req.param("id"), payload.id);
...
expenseRoute.get(
  ...
  async (c: Context) => {
    ...
    const { expenses, totalExpenses, meta } = await expenseService.getExpenses(
      payload.id,
      {
        limit,
        offset,
