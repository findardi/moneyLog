    ...
    try {
      await db.transaction(async (trx) => {
        ...
        const result = await trx
          .select()
          .from(token)
          .where(and(eq(token.username, username), eq(token.token, validToken)))
          .then((result) => result[0]);
        if (!result) {
        ...
        if (
          result.type !== tokenType.ACTIVATION &&
          result.type !== tokenType.RESET
        ) {
          throw new BadRequest("Invalid token");
        }
        ...
        const updateUser = await trx
          ...
          .where(eq(users.username, username))
          .returning({ username: users.username, isactive: users.isActive })
          .then((result) => result[0]);
        ...
        await trx
          .delete(token)
          .where(
            and(eq(token.username, username), eq(token.token, validToken)),
          );
