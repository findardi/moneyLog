<script lang="ts">
    ...
    interface alertProps {
        formCreate: SuperValidated<createSpendingLimitDto>;
        formEdit: SuperValidated<updateSpendingLimitDto>;
    }

    let { formCreate, formEdit }: alertProps = $props();
    ...
    const connection = source("/api/dashboard/alert", {
        close({ connect }) {
            if (!noBudget) {
                ...
                console.log(
                    "No budget limit set, connection closed permanently",
                );
            }
        },
    });

    const connectedData = connection
        .select("connected")
        .json(({ error, raw, previous }) => {
            if (!raw || raw.trim() === "") return previous;
            console.error("Connected JSON parse error:", error, "Raw:", raw);
            return previous;
        });

    const notificationData = connection
        .select("notification")
        .json<AlertNotification>(({ error, raw, previous }) => {
            if (!raw || raw.trim() === "") return previous;
            console.error("Notification JSON parse error:", error, "Raw:", raw);
            return previous;
        });

    const errorData = connection
        .select("error")
        .json<ErrorMessage>(({ previous }) => previous);

    const noBudgetData = connection
        .select("no_budget")
        .json<{ message: string }>(({ error, raw, previous }) => {
            if (!raw || raw.trim() === "") return previous;
...
<div
    class="w-full h-full border-4 border-stone-900 bg-white shadow-[4px_4px_0px_0px_rgba(41,37,36,1)] p-4"
>
    <div class="flex flex-col h-full">
        ...
        <div class="mb-4">
            <div class="flex items-center justify-between">
                ...
                {#if $notificationData}
                    <div class="flex gap-2">
                        ...
                        <button
                            onclick={() =>
                                openDeleteModal($notificationData.id)}
            ...
            {#if $notificationData}
                <div
                    class="mt-2 p-3 border-2 border-stone-900 {$notificationData.isAlert
                        ? 'bg-red-100'
                        : 'bg-green-100'}"
                >
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-2">
                    <div class="flex justify-between items-center">
                        ...
                        >
                        <span
                            class="text-xs font-bold {$notificationData.isAlert
                                ? 'text-red-600'
                                : 'text-green-600'}"
                        >
                    ...
                    <div
                        class="w-full h-8 border-4 border-stone-900 bg-stone-100 relative overflow-hidden"
                    >
                        <div
                            class="h-full transition-all duration-500 {$notificationData.isAlert
                                ? 'bg-red-500'
                                : 'bg-green-500'}"
                            style="width: {Math.min(
                                $notificationData.currentPercentage,
                                100,
                            )}%"
                        ></div>
                    ...
                    <div class="flex justify-between items-center text-xs">
                        <span class="font-bold text-stone-600"
                            >{formatCurrency(
                                $notificationData.currentSpending,
                            )}</span
                        >
                        <span class="font-bold text-yellow-600"
                            >Alert: {$notificationData.alertPercentage}%</span
                        >
                        <span class="font-bold text-stone-900"
                            >{formatCurrency(
                                $notificationData.maxSpending,
                            )}</span
                        >
                    </div>
                </div>
        ...
        {#if $noBudgetData}
            <div class="flex flex-col space-y-3">
                <p class="text-sm text-stone-600">
                    {$noBudgetData.message ||
                        "No budget limit configured. Create one to track spending."}
                </p>
                <button
...
<ModalDeleteSpendComponent
    bind:isOpen={isDeleteOpen}
    onClose={closeDeleteModal}
    spendingId={deleteSpendingId}
/>
